...
- **Activity feed sort toggle** — Users can now flip the activity feed between oldest-first and latest-first on any resource; preference is stored per-user on `users.activity_sort_order` (default `asc`) and auto-saves via the generic `AutoSaveController`. `HasActivityFeed::getActivityFeed()` resolves the direction from the authenticated user or an explicit override, and `PartialController::activityFeed()` passes the preference through so the sort survives page reloads and partial refreshes (PLAN-032 Phase 2)
- **Undated tasks, follow-ups, and meetings** — Meetings can now be created and edited without `scheduled_at` (migration makes the column nullable, `MeetingRequest` switches from `required` to `nullable`). Undated meetings and follow-ups surface in a dedicated "Undated / Prep" section at the bottom of their respective list pages; `FollowUp` date scopes (`overdue`, `dueToday`, `dueThisWeek`, `upcoming`) now `whereNotNull` guard their queries so null-dated records are no longer incorrectly bucketed or breaking sort (PLAN-032 Phase 3)
- **Monday-first calendar with weekend tint** — The Blade calendar component now seeds its 7-day loop from `Carbon::now()->startOfWeek(Carbon::MONDAY)` so the week runs Mon-Sun; Saturday and Sunday cells carry a warm-stone background in light mode and a legible tinted background in dark mode (PLAN-032 Phase 4)
- **Flatpickr Monday-first and themed dark mode** — All date pickers (`date-picker.ts`, `auto-save-date-picker.ts`) now consume a shared `flatpickr-defaults.ts` util that sets `locale.firstDayOfWeek = 1`; added dark-mode styling for the month `<select>`, year input, and arrow controls so the popover is legible against the Rivendell dark theme; weekend column headers tint to distinguish Sat/Sun (PLAN-032 Phase 7)
- **Dashboard upcoming calendar — smarter limit and day divider** — `DashboardController::index()` and `PartialController::dashboardCalendar()` apply the user's configured `dashboard_upcoming_meetings` limit (default 5) to timed events only; all-day events are always shown and render at the top of each day group without consuming a slot. A single `elvish-divider-leaf` renders once between today and the first later day group when both are present (PLAN-032 Phases 5 & 6)
- **Activity-sort toggle Alpine component** — New `activity-sort-toggle.ts` (registered in `resources/js/app.ts`) drives the asc/desc control on the activity feed, wired to `AutoSaveController` for the `users.activity_sort_order` field and refreshing the feed partial without a full page reload
...
- **Task overdue scope null-safety regression guard** — `Task::scopeOverdue()` was already null-safe, but behavior was undocumented in tests; added a regression test (`TaskOverdueNullSafeTest`) so future refactors can't silently reintroduce a null-deadline bug
- **Calendar single-digit day rendering** — Added a regression test ensuring single-digit day numbers (1-9) render correctly in the calendar component after the Monday-first refactor
- **Calendar empty trailing week row hidden** — The Flatpickr 6th week row is now collapsed via CSS when it contains only next-month placeholder days, removing the visually empty row reported on months like April 2026
- **Keyboard shortcut help overlay refinements** — Adjusted `keyboard-shortcuts.ts` and `shortcut-help.ts` so the help overlay and shortcut dispatch cooperate cleanly with the new global search focus rules
- **Local retranscribe ignored with-speakers toggle** — `processLocally()` in the transcription-viewer component branched on the feature flag `diarizationEnabled` instead of the user's `processingMode` toggle, so "Text only" always produced a diarized result. Now uses `processingMode === 'diarize'` so the toggle routes to `/transcribe(/stream)` or `/diarize(/stream)` correctly
- **Retranscribing text-only did not overwrite prior diarization** — `ClientTranscriptionController::storeResult()` only touched `diarized_content`/`diarization_status`/`diarization_error` when the incoming `diarized_content` was non-null, so switching from "With speakers" to "Text only" left stale speaker segments in the database and the UI kept rendering them. Those three fields are now always written on save; null `diarized_content` clears the diarization state
- **Local processing overlay showed no progress bar** — The blocking modal during local transcription only showed a spinner and elapsed time; the real progress bar sat behind the backdrop. The overlay now renders the progress bar inside the card when `overallProgressPercent` is available
- **SSE progress bar never reached 100%** — Whisper's final `seg.end` is often slightly under `info.duration` (silence trim/rounding), so the last emitted progress value was ~0.97-0.99; the `result` event arrived afterward without touching progress. The client now sets `streamProgress = 1` on `result` so the bar hits 100% before the modal closes
...
- **`SPEECH_SERVER_ENABLED` env var** — New independent flag to enable/disable server-side speech processing; decoupled from `MEETING_TRANSCRIPTION_ENABLED` and `MEETING_DIARIZATION_ENABLED` so server and local modes can be toggled independently

### Fixed

- **Private Network Access** — Speech service Docker container now responds with `Access-Control-Allow-Private-Network: true` header, allowing browsers to connect from public origins to local loopback services
...
- **AI error messages** — Invalid JSON errors now include the first 200 characters of the raw API response for easier debugging

## [1.9.1] - 2026-03-22

### Fixed

- **About page XSS** — Changelog entries containing HTML-like text (e.g. `<select>`) were rendered as actual HTML elements; now escaped before rendering

### Changed

- **Meetings index** — Past meetings are no longer loaded by default; use the "Show past" filter toggle to include them
- **About page changelog** — Inline code (backtick-wrapped text) in changelog entries now renders as styled `<code>` tags
