import type { EmailLink } from '../types/models';
 ...
 */
function emailActions(emailId: number, initialLinks: EmailLink[] = [], canCreateBila: boolean = false): Record<string, unknown> {
    return {
        emailId,
        ...
        /**
         * Create a resource of the given type from this email.
         */
        async createResource(this: { emailId: number; links: LinkBadge[]; menuOpen: boolean; isLoading: boolean; errorMessage: string }, type: string): Promise<void> {
            ...
            try {
                const response = await fetch(`/api/v1/emails/${this.emailId}/create/${type}`, {
                    method: 'POST',
                    headers: {
                ...
                const json = await response.json() as { success: boolean; data?: { link?: EmailLink }; message?: string };
        ...
        /**
         * Remove a link between this email and a resource (does not delete the resource).
         */
        async unlinkResource(this: { emailId: number; links: LinkBadge[]; isLoading: boolean }, linkId: number): Promise<void> {
            if (this.isLoading) return;
            this.isLoading = true;

            try {
                const response = await fetch(`/api/v1/emails/${this.emailId}/links/${linkId}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': getCSRFToken(),
                        'Accept': 'application/json',
