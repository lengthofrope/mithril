...
/**
 * Map a CalendarEventLink to a display-friendly badge object.
 */
function mapLinkToBadge(link: CalendarEventLink): LinkBadge {
 ...
 */
function calendarEventActions(eventId: number, initialLinks: CalendarEventLink[] = [], canCreateBila: boolean = true): Record<string, unknown> {
    return {
        eventId,
        ...
        /**
         * Remove a link between this event and a resource (does not delete the resource).
         */
        async unlinkResource(this: { eventId: number; links: LinkBadge[]; isLoading: boolean }, linkId: number): Promise<void> {
            if (this.isLoading) return;
            this.isLoading = true;

            try {
                const response = await fetch(`/api/v1/calendar-events/${this.eventId}/links/${linkId}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': getCSRFToken(),
                        'Accept': 'application/json',
