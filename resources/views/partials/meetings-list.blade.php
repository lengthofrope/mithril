...
{{-- Undated / Prep meetings — collapsed section at the bottom --}}
@if(($undatedMeetings ?? collect())->isNotEmpty())
    <div x-data="toggleState({ open: false })">
        ...
        >
            <h2 class="text-base font-semibold text-gray-800 dark:text-white/90">Undated / Prep</h2>
            <span class="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-600 dark:bg-gray-800 dark:text-gray-400">
                {{ ($undatedMeetings ?? collect())->count() }}
            </span>
            <svg
                class="ml-auto h-4 w-4 shrink-0 text-gray-400 transition-transform dark:text-gray-500"
        ...
        <div x-show="open" x-cloak>
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                @foreach($undatedMeetings as $meeting)
                    <a
                        href="{{ route('meetings.show', $meeting->id) }}"
                        class="group flex items-center gap-4 rounded-xl border border-gray-200 bg-white p-4 transition hover:border-gray-300 hover:shadow-sm dark:border-gray-800 dark:bg-white/[0.03] dark:hover:border-gray-700"
                    >
