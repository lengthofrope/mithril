    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-2">
      ...
      <div class="space-y-6">
        ...
        @if(auth()->user()->hasMicrosoftConnection())
            <div class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="border-b border-gray-100 px-5 py-4 dark:border-gray-800">
                    <h2 class="text-sm font-semibold text-gray-800 dark:text-white/90">E-mail sources</h2>
                </div>
                <div
                    class="p-5 space-y-4"
                    x-data="{
                        ...
                        async save() {
                            ...
                            this.error = '';
                            try {
                                const response = await fetch('{{ route('settings.updateEmailSources') }}', {
                                    method: 'PATCH',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content,
                ...
                >
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        Choose which emails to sync from Outlook. At least one source must be active.
                    </p>

                    <label class="flex items-center justify-between gap-4 cursor-pointer">
                        <div>
                            <p class="text-sm font-medium text-gray-800 dark:text-white/90">Flagged emails</p>
                            <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Emails you've flagged in Outlook</p>
                        </div>
                        <input
                            type="checkbox"
                            x-model="flagged"
                    ...
                    <label class="flex items-center justify-between gap-4 cursor-pointer">
                        <div>
                            <p class="text-sm font-medium text-gray-800 dark:text-white/90">Unread emails</p>
                            <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">All unread emails from your inbox</p>
                        </div>
                        <input
                            type="checkbox"
                            x-model="unread"
